...
候选列表仍按 `select_modes` 配置渲染；歌曲发送方式为单选，失败后不会自动切换。
...
1. record_link（语音链接，默认）
2. file_link（文件链接）

语音链接会直接向 aiocqhttp/OneBot 发送远程 URL，保留音源压缩格式，避免 AstrBot 将整首歌曲转换为 WAV Base64。已知音频 `Content-Length` 超过 15 MiB 时会拒绝发送。
...
| `<平台名>点歌 <歌名>` | 指定平台点歌     |
| 点歌 `<歌名> <序号>`  | 直接选择搜索结果 |
| `<歌曲列表序号> <模式>` | 播放歌单歌曲，模式：语音/语音链接/文件/文件链接 或者 1/2
---
...
- `search_and_play_music`：用户想听歌曲或某位歌手的歌时执行搜索；单个结果直接播放，多个结果按 `select_modes` 发送候选列表。
- `confirm_music_selection`：用户收到候选列表并回复序号后，播放当前会话中对应的歌曲。

`search_and_play_music` 只接收 `query` 参数，平台始终从 `default_player_name` 读取。工具说明会要求模型从用户原话提取歌曲名（优先）或歌手名，不要自行补充搜索词；例如“我想听鸣潮的歌”传入“鸣潮”，“来一首小小奇迹”传入“小小奇迹”。插件代码不会改写 `query`。

候选列表按“会话 + 用户”隔离，并遵循 `timeout` 配置。超时后会清理候选状态并提示用户；用户对过期列表继续回复时，确认工具会把超时状态返回给 LLM。最终歌曲按照 `send_modes` 选择的唯一方式发送。
